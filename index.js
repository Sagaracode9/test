// ==UserScript==
// @name         saBot Claimer MultiAccount (HTML Rebuild, Auto Account, Turnstile Random)
// @namespace    http://tampermonkey.net/
// @version      1.2
// @description  Multi-account, nama akun dari Stake, random turnstile, auto UI modern
// @author       Gemini AI
...
(function() {
    ...
    document.getElementById('fb-connectAPI').onclick = async function() {
        ...
        let viphost = "-", faucet = "-";
        try {
            const query = `query VipMeta { user { vipInfo { host { name contactHandle contactLink email availableDays } } reload: faucet { value active } } }`;
            const res = await fetch(API_URL, {
                method: "POST",
                headers: { "Content-Type": "application/json", "x-access-token": activeApiKey },
                body: JSON.stringify({ query })
    ...
    document.getElementById('fb-btnCheckBonus').onclick = async function() {
        ...
        const variables = { code, couponType: couponType.toLowerCase() };
        try {
            const res = await fetch(API_URL, {
                method: "POST",
                headers: { "Content-Type": "application/json", "x-access-token": activeApiKey },
